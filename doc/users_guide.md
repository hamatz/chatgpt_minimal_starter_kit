...
- オープンソース: プラグインのソースコードが公開されており、学習や改造に最適。
...
1. CraftForgeの公式ウェブサイトから、最新のCraftForge実行ファイルをダウンロードします。
...
CraftForgeを起動すると、ホーム画面が表示されます。ホーム画面には、インストールされているプラグインのアイコンが表示されます。プラグインのアイコンをクリックすると、そのプラグインが起動します。
...
3. 確認ダイアログが表示されるので、「削除」ボタンをクリックします。
...
CraftForgeでは、プラグインのバージョン管理が行われます。新しいバージョンのプラグインをインストールする際、ユーザーが明示的に古いバージョンを削除しない限り、新しいバージョンは別のプラグインとしてインストールされます。この仕様により、ユーザーがカスタマイズしたプラグインが誤って上書きされることを予防しています。

プラグインを更新するには、以下の手順に従います。

1. 更新されたプラグインのzipファイルを入手します。 
2. CraftForgeのホーム画面を開きます。 
3. 古いバージョンのプラグインを削除する場合は、以下の手順を実行します。  
   a. 削除するプラグインのアイコンを長押しします。 
   b. 確認ダイアログが表示されるので、「削除」ボタンをクリックします。  
4. 画面下部にある「Install Plugin」ボタンをクリックします。 
5. ファイル選択ダイアログが表示されるので、更新されたプラグインのzipファイルを選択します。 
6. 「開く」または「選択」ボタンをクリックします。 

CraftForgeは、新しいバージョンのプラグインを別のプラグインとしてインストールします。古いバージョンのプラグインを削除しなかった場合、両方のバージョンのプラグインがホーム画面に表示されます。

ただし、CraftForgeは、プラグインの自動更新機能を提供していません。プラグインの更新は、ユーザーが手動で行う必要があります。
...
1. CraftForgeのホーム画面を開きます。
2. 「Shared Folder Manager」プラグインのアイコンをクリックします。
3. 共有するフォルダを選択し、アクセス権を付与するプラグインを指定します。
4. 「保存」ボタンをクリックして、変更を保存します。

## 4.3. その他のシステムプラグイン

現在、CraftForgeには設定プラグインと共有フォルダ管理プラグイン以外のシステムプラグインは含まれていません（厳密にはUIコンポーネント管理プラグインが存在していますが、UIコンポーネントのカタログとしてサポートされているUI部品をカタログ的に見えるだけのもので、プラグイン単体では特に操作できることがありませんので割愛します）。ただし、将来的には、システムの機能拡張に伴い、新たなシステムプラグインが追加される可能性があります。
...
IntentConductorを利用することで、あるプラグインから別のプラグインの機能を呼び出したり、データを受け渡したりすることができます。例えば、「PDFの最後の数ページにあるディスクレイマー部分がデータ検索の邪魔になるので指定されたページ以降の内容を除去するプラグイン」で処理したPDFを、「PDFの中のテキストを抽出してベクトルDBへの登録を行うプラグイン」に渡して処理を続行するといったことが可能です。

このような連携により、ユーザーは目的に応じてプラグインを組み合わせ、より強力で柔軟な処理を実現できます。

### 5.4.2. 処理の自動化

IntentConductorのPipe機能を使うと、複数のプラグインの処理を連結して自動化することができます。例えば、特定のフォルダ内にある多数のPDFファイルに対して、以下のような一連の処理を自動で行うことができます。

1. 各PDFファイルから指定されたページ以降の内容を削除する
2. ページ削除後のPDFからテキストを抽出する
3. 抽出したテキストをベクトルDBに登録する

このように、IntentConductorを活用することで、ユーザーは手作業では時間がかかる反復的な処理を自動化し、効率を大幅に改善できます。

プラグインの連携と自動化の設定には、少し技術的な知識が必要になることがあります。プラグインの説明書を参照したり、プラグイン開発者にアドバイスを求めたりすることをお勧めします。
...
- Q: プラグインの開発にはどのようなスキルが必要ですか？ 
  A: プラグインの開発には、Python言語の基本的な知識が必要です。また、CraftForgeが提供するUIコンポーネントやAPIを理解している必要があります。
