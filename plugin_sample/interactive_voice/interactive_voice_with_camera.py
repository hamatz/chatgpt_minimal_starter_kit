...
class InteractiveVoicePlugin(MultimediaPluginInterface):
    ...
    def start_recording(self):
        ...
        while self.is_recording:
            ...
            self.api.multimedia.save_image("camera_image.jpg", camera_image)
            image_base64 = self.api.multimedia.encode_image("camera_image.jpg")
            ...
            # ユーザーの録音内容をVision APIの質問として使用する場合
            vision_response = self.api.multimedia.vision_api_request(image_base64, transcribed_text)
            self.chat_view.controls.append(ft.Text(f"Vision API Response: {vision_response}"))
            self.page.update()

            #音声コミュニケーションだけにしたい場合はこちらを有効化する
            ...
            saved_file_path = self.text_to_speech(vision_response)
